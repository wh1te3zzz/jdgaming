...
3.加购5京豆
  (默认不加购 如需加购请设置环境变量[opencard_addSku7]为"true"
4.浏览店铺1豆/个
5.浏览商品1豆/个
...
[task_local]
#12.18~12.30 京选大牌 超会宠你
30 7 23-30/2 12 * https://github.com/wh1te3zzz/jdgaming/blob/main/jd_active_OpenCard7.js, tag=12.18~12.30 京选大牌 超会宠你, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku7 ? process.env.opencard_addSku7 : `${opencard_addSku}`) : ($.getdata('opencard_addSku7') ? $.getdata('opencard_addSku7') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard7 ? process.env.opencard7 : `${opencard}`) : ($.getdata('opencard7') ? $.getdata('opencard7') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait7 ? process.env.openwait7 : `${openwait}`) : ($.getdata('openwait7') ? $.getdata('openwait7') : `${openwait}`);
...
!(async () => {
  if ($.isNode()) {
    if(opencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[opencard7]为"true"')
    }
    if(opencard+"" != "true"){
      return
    }
  ...
  $.shareUuid = "179ba2f5a1b4401f998ee2bb2e4a4afa"
  ...
  try {
    ...
    if(!$.addSku && !$.outFlag){
      if(opencard_addSku+"" == "true"){
        ...
        console.log('如需加购请设置环境变量[opencard_addSku7]为"true"');
      }
    }
    $.runFalag = true
    $.log("浏览店铺: " + $.toShop)
