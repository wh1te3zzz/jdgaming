...
3.关注5京豆
4.加购5京豆
  (默认不加购 如需加购请设置环境变量[opencard_addSku19]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw19]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw19="3"
填非数字会全都抽奖
...
All变量适用
————————————————
入口：[ 1.8~1.15 大牌联合 宠爱有礼 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dzlhkklblzms20220108&shareUuid=96e73739613f48978c755ad6959584da)]
...
[task_local]
#1.8~1.15 大牌联合 宠爱有礼
30 13 9-15/3 1 * https://github.com/wh1te3zzz/jdgaming/blob/main/jd_active_OpenCard19.js, tag=1.8~1.15 大牌联合 宠爱有礼, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku19 ? process.env.opencard_addSku19 : `${opencard_addSku}`) : ($.getdata('opencard_addSku19') ? $.getdata('opencard_addSku19') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard19 ? process.env.opencard19 : `${opencard}`) : ($.getdata('opencard19') ? $.getdata('opencard19') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait19 ? process.env.openwait19 : `${openwait}`) : ($.getdata('openwait19') ? $.getdata('openwait19') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw19 ? process.env.opencard_draw19 : opencard_draw) : ($.getdata('opencard_draw19') ? $.getdata('opencard_draw19') : opencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(opencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[opencard19]为"true"')
    }
    if(opencard+"" != "true"){
      return
    }
  ...
  $.shareUuid = "96e73739613f48978c755ad6959584da"
  ...
  try {
    ...
    if(!$.addCart && !$.outFlag){
      if(opencard_addSku+"" == "true"){
        ...
        console.log('如需加购请设置环境变量[opencard_addSku19]为"true"');
      }
    }
    if(flag){
      await takePostRequest('activityContent');
    ...
    }else console.log('如需抽奖请设置环境变量[opencard_draw19]为"3" 3为次数');
