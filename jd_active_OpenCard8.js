...
2.开10张 成功开1张 可能获得5京豆
  全部开完获得1次抽奖
3.关注10京豆 获得1次抽奖
4.加购5京豆 获得1次抽奖(默认不加购 如需加购请设置环境变量[opencard_addSku8]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw8]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw8="3"
填非数字会全都抽奖
...
[task_local]
#12.23~1.2 联合开卡
30 8 26-30/2,2 12,1 * https://github.com/wh1te3zzz/jdgaming/blob/main/jd_active_OpenCard8.js, tag=12.23~1.2 联合开卡, enabled=true
...
const $ = new Env('12.23~1.2 联合开卡-8');
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku8 ? process.env.opencard_addSku8 : `${opencard_addSku}`) : ($.getdata('opencard_addSku8') ? $.getdata('opencard_addSku8') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard8 ? process.env.opencard8 : `${opencard}`) : ($.getdata('opencard8') ? $.getdata('opencard8') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait8 ? process.env.openwait8 : `${openwait}`) : ($.getdata('openwait8') ? $.getdata('openwait8') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw8 ? process.env.opencard_draw8 : opencard_draw) : ($.getdata('opencard_draw8') ? $.getdata('opencard_draw8') : opencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(opencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[opencard8]为"true"')
    }
    if(opencard+"" != "true"){
      return
    }
...
async function run() {
  try {
    ...
    if(opencard_addSku+"" == "true"){
      ...
      console.log('如需加购请设置环境变量[opencard_addSku8]为"true"');
    }
    await takePostRequest('activity_load');
    if(opencard_draw+"" !== "0"){
      $.runFalag = true
    ...
    }else console.log('如需抽奖请设置环境变量[opencard_draw8]为"3" 3为次数');
