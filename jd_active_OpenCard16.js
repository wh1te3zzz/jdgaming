...
2.开10张卡 成功开1张 获得5豆
3.关注5豆 
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw16]为"3"
...
All变量适用
————————————————
入口：[ 1.8~1.27 集年味卡 享年味盛典 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=cd20220108iu890dmnhimj01ied&shareUuid=3c13b6530edc4086b19e20b920e4ce8e)]

请求太频繁会被黑ip
过10分钟再执行

cron:30 6 28,7-27/3 1 *
============Quantumultx===============
[task_local]
#1.8~1.27 集年味卡 享年味盛典
30 6 28,7-28/3 1 * https://github.com/wh1te3zzz/jdgaming/blob/main/jd_active_OpenCard16.js, tag=1.8~1.27 集年味卡 享年味盛典, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku16 ? process.env.opencard_addSku16 : `${opencard_addSku}`) : ($.getdata('opencard_addSku16') ? $.getdata('opencard_addSku16') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard16 ? process.env.opencard16 : `${opencard}`) : ($.getdata('opencard16') ? $.getdata('opencard16') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait16 ? process.env.openwait16 : `${openwait}`) : ($.getdata('openwait16') ? $.getdata('openwait16') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw16 ? process.env.opencard_draw16 : opencard_draw) : ($.getdata('opencard_draw16') ? $.getdata('opencard_draw16') : opencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(opencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[opencard16]为"true"')
    }
    if(opencard+"" != "true"){
      return
    }
  ...
  $.shareUuid = "3c13b6530edc4086b19e20b920e4ce8e"
...
async function run() {
  try {
    ...
    }else console.log('如需抽奖请设置环境变量[opencard_draw16]为"3" 3为次数');
