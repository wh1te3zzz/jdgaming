...
[task_local]
#12.18~12.24 联合开卡
30 6 22-24/2 12 * https://github.com/wh1te3zzz/jdgaming/blob/main/jd_active_OpenCard6.js, tag=12.18~12.24 联合开卡, enabled=true
...
const $ = new Env('12.18~12.24 联合开卡-6');
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku6 ? process.env.opencard_addSku6 : `${opencard_addSku}`) : ($.getdata('opencard_addSku6') ? $.getdata('opencard_addSku6') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard6 ? process.env.opencard6 : `${opencard}`) : ($.getdata('opencard6') ? $.getdata('opencard6') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait6 ? process.env.openwait6 : `${openwait}`) : ($.getdata('openwait6') ? $.getdata('openwait6') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw6 ? process.env.opencard_draw6 : opencard_draw) : ($.getdata('opencard_draw6') ? $.getdata('opencard_draw6') : opencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(opencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[opencard6]为"true"')
    }
    if(opencard+"" != "true"){
      return
    }
...
async function run() {
  try {
    ...
    if(opencard_addSku+"" == "true"){
      ...
      console.log('如需加购请设置环境变量[opencard_addSku6]为"true"');
    }
    await takePostRequest('activity_load');
