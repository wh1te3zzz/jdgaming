...
2.开10张卡 成功开1张 获得5豆
3.关注5豆 
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw17]为"3"
...
All变量适用
————————————————
入口：[ 1.5~1.31 年货盛宴 春节集福攻略 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=cd20220105fff606x19uj4vijetkg&shareUuid=41db2fc510e24a7dbde3941e603ab975)]

请求太频繁会被黑ip
过10分钟再执行

cron:30 4 1,11-31/3 1,2 *
============Quantumultx===============
[task_local]
#1.5~1.31 年货盛宴 春节集福攻略
30 4 1,11-31/3 1,2 * https://github.com/wh1te3zzz/jdgaming/blob/main/jd_active_OpenCard17.js, tag=1.5~1.31 年货盛宴 春节集福攻略, enabled=true

*/
let opencard_addSku = "false"
let opencard = "false"
let opencard_compositeCard = "false" // 集卡 默认不集卡 需要请设置"true"
let openwait = "0"
let opencard_draw = "0"
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku17 ? process.env.opencard_addSku17 : `${opencard_addSku}`) : ($.getdata('opencard_addSku17') ? $.getdata('opencard_addSku17') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard17 ? process.env.opencard17 : `${opencard}`) : ($.getdata('opencard17') ? $.getdata('opencard17') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait17 ? process.env.openwait17 : `${openwait}`) : ($.getdata('openwait17') ? $.getdata('openwait17') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw17 ? process.env.opencard_draw17 : opencard_draw) : ($.getdata('opencard_draw17') ? $.getdata('opencard_draw17') : opencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(opencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[opencard17]为"true"')
    }
    if(opencard+"" != "true"){
      return
    }
  ...
  $.shareUuid = "41db2fc510e24a7dbde3941e603ab975"
...
async function run() {
  try {
    ...
    }else console.log('如需抽奖请设置环境变量[opencard_draw17]为"3" 3为次数');
