...
3.关注5豆 
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw2]为"3"
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku2 ? process.env.opencard_addSku2 : `${opencard_addSku}`) : ($.getdata('opencard_addSku2') ? $.getdata('opencard_addSku2') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard2 ? process.env.opencard2 : `${opencard}`) : ($.getdata('opencard2') ? $.getdata('opencard2') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait2 ? process.env.openwait2 : `${openwait}`) : ($.getdata('openwait2') ? $.getdata('openwait2') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw2 ? process.env.opencard_draw2 : opencard_draw) : ($.getdata('opencard_draw2') ? $.getdata('opencard_draw2') : opencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(opencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[opencard2]为"true"')
    }
    if(opencard+"" != "true"){
      return
    }
  ...
  $.shareUuid = "9357b56d03c8423a9cc048fde19f82a0"
...
async function run() {
  try {
    ...
    }else console.log('如需抽奖请设置环境变量[opencard_draw2]为"3" 3为次数');
