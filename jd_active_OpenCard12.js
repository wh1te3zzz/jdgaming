...
3.关注10京豆 
4.逛会场5京豆 
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw12]为"3" 
填写要抽奖的次数 不足已自身次数为准 
opencard_draw12="3" 
填非数字会全都抽奖 
...
All变量适用 
———————————————— 
入口：[ 1.6～1.31 会员狂欢日 (https://lzdz1-isv.isvjcloud.com/dingzhi/majorsuit/memberday/activity?activityId=dz2112100022468201&shareUuid=f3dcab90a6144232ad59c6a03e083004)] 

请求太频繁会被黑ip 
过10分钟再执行 

cron:30 3 6-31/2,31 1 * 
============Quantumultx=============== 
[task_local] 
#1.6～1.31 会员狂欢日 
30 3 6-31/2,31 1 * https://github.com/wh1te3zzz/jdgaming/blob/main/jd_active_OpenCard12.js, tag=1.6～1.31 会员狂欢日, enabled=true 
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku12 ? process.env.opencard_addSku12 : `${opencard_addSku}`) : ($.getdata('opencard_addSku12') ? $.getdata('opencard_addSku12') : `${opencard_addSku}`); 
...
opencard = $.isNode() ? (process.env.opencard12 ? process.env.opencard12 : `${opencard}`) : ($.getdata('opencard12') ? $.getdata('opencard12') : `${opencard}`); 
...
openwait = $.isNode() ? (process.env.openwait12 ? process.env.openwait12 : `${openwait}`) : ($.getdata('openwait12') ? $.getdata('openwait12') : `${openwait}`); 
...
opencard_draw = $.isNode() ? (process.env.opencard_draw12 ? process.env.opencard_draw12 : opencard_draw) : ($.getdata('opencard_draw12') ? $.getdata('opencard_draw12') : opencard_draw); 
...
!(async () => { 
 if ($.isNode()) { 
 if(opencard+"" != "true"){ 
 console.log('如需执行脚本请设置环境变量[opencard12]为"true"') 
 } 
 if(opencard+"" != "true"){ 
 return 
 } 
 ...
 $.shareUuid = "f3dcab90a6144232ad59c6a03e083004" 
...
async function run() { 
 ...
 }else console.log('如需抽奖请设置环境变量[opencard_draw12]为"3" 3为次数'); 
