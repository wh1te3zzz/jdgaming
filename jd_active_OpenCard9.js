...
3.关注5京豆
4.加购5京豆
  (默认不加购 如需加购请设置环境变量[opencard_addSku9]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw9]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw9="3"
填非数字会全都抽奖
...
All变量适用
————————————————
入口：[ 12.31~1.9 大牌联合 年货盛宴 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dzlhkklblzms20211227&shareUuid=7b80abdc211643af87306725aaf03821)]
...
[task_local]
#12.31~1.9 大牌联合 年货盛宴
30 1 3-9/2 1 * https://github.com/wh1te3zzz/jdgaming/blob/main/jd_active_OpenCard9.js, tag=12.31~1.9 大牌联合 年货盛宴, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku9 ? process.env.opencard_addSku9 : `${opencard_addSku}`) : ($.getdata('opencard_addSku9') ? $.getdata('opencard_addSku9') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard9 ? process.env.opencard9 : `${opencard}`) : ($.getdata('opencard9') ? $.getdata('opencard9') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait9 ? process.env.openwait9 : `${openwait}`) : ($.getdata('openwait9') ? $.getdata('openwait9') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw9 ? process.env.opencard_draw9 : opencard_draw) : ($.getdata('opencard_draw9') ? $.getdata('opencard_draw9') : opencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(opencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[opencard9]为"true"')
    }
    if(opencard+"" != "true"){
      return
    }
  ...
  $.shareUuid = "7b80abdc211643af87306725aaf03821"
  ...
  try {
    ...
    if(!$.addCart && !$.outFlag){
      if(opencard_addSku+"" == "true"){
        ...
        console.log('如需加购请设置环境变量[opencard_addSku9]为"true"');
      }
    }
    if(flag){
      await takePostRequest('activityContent');
    ...
    }else console.log('如需抽奖请设置环境变量[opencard_draw9]为"3" 3为次数');
