...
2.开10张卡 成功开1张 获得5豆
3.关注5豆 
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw15]为"3"
...
All变量适用
————————————————
入口：[ 1.7~1.26 集“年味卡”，享年货盛宴 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=cd20220107vcowv119979tm7p&shareUuid=1a44bf0855fd493395d6d5fb08bb4d49)]

请求太频繁会被黑ip
过10分钟再执行

cron:30 7 27,7-27/3 1 *
============Quantumultx===============
[task_local]
#1.7~1.26 集“年味卡”，享年货盛宴
30 7 27,7-27/3 1 * https://github.com/wh1te3zzz/jdgaming/blob/main/jd_active_OpenCard15.js, tag=1.7~1.26 集“年味卡”，享年货盛宴, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku15 ? process.env.opencard_addSku15 : `${opencard_addSku}`) : ($.getdata('opencard_addSku15') ? $.getdata('opencard_addSku15') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard15 ? process.env.opencard15 : `${opencard}`) : ($.getdata('opencard15') ? $.getdata('opencard15') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait15 ? process.env.openwait15 : `${openwait}`) : ($.getdata('openwait15') ? $.getdata('openwait15') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw15 ? process.env.opencard_draw15 : opencard_draw) : ($.getdata('opencard_draw15') ? $.getdata('opencard_draw15') : opencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(opencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[opencard15]为"true"')
    }
    if(opencard+"" != "true"){
      return
    }
  ...
  $.shareUuid = "1a44bf0855fd493395d6d5fb08bb4d49"
...
async function run() {
  try {
    ...
    }else console.log('如需抽奖请设置环境变量[opencard_draw15]为"3" 3为次数');
