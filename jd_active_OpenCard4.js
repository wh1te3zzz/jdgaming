...
3.关注5豆 
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw4]为"3"
...
All变量适用
————————————————
入口：[ 12.20~1.5 双旦寻宝 玩转大富翁 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dfw2021121701vrb6n1c0ilo0uw&shareUuid=fba827760e6e469f8ce6a19a94b3a019)]

请求太频繁会被黑ip
过10分钟再执行

cron:30 4 21-31/2,2-5/2 12,1 *
============Quantumultx===============
[task_local]
#12.20~1.5 双旦寻宝 玩转大富翁
30 4 21-31/2,2-5/2 12,1 * https://github.com/wh1te3zzz/jdgaming/blob/main/jd_active_OpenCard4.js, tag=12.20~1.5 双旦寻宝 玩转大富翁, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku4 ? process.env.opencard_addSku4 : `${opencard_addSku}`) : ($.getdata('opencard_addSku4') ? $.getdata('opencard_addSku4') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard4 ? process.env.opencard4 : `${opencard}`) : ($.getdata('opencard4') ? $.getdata('opencard4') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait4 ? process.env.openwait4 : `${openwait}`) : ($.getdata('openwait4') ? $.getdata('openwait4') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw4 ? process.env.opencard_draw4 : opencard_draw) : ($.getdata('opencard_draw4') ? $.getdata('opencard_draw4') : opencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(opencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[opencard4]为"true"')
    }
    if(opencard+"" != "true"){
      return
    }
  ...
  $.shareUuid = "fba827760e6e469f8ce6a19a94b3a019"
...
async function run() {
  try {
    ...
    }else console.log('如需抽奖请设置环境变量[opencard_draw4]为"3" 3为次数');
