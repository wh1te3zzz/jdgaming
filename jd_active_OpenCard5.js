...
2.开14张 成功开1张 可能获得5京豆
  全部开完获得1次抽奖
3.关注10京豆 获得1次抽奖
4.加购5京豆 获得1次抽奖(默认不加购 如需加购请设置环境变量[opencard_addSku5]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw5]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw5="3"
填非数字会全都抽奖
...
[task_local]
#12.21~12.30 联合开卡
30 5 22-30/2 12 * https://github.com/wh1te3zzz/jdgaming/blob/main/jd_active_OpenCard5.js, tag=12.21~12.30 联合开卡, enabled=true
...
const $ = new Env('12.21~12.30 联合开卡-5');
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku5 ? process.env.opencard_addSku5 : `${opencard_addSku}`) : ($.getdata('opencard_addSku5') ? $.getdata('opencard_addSku5') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard5 ? process.env.opencard5 : `${opencard}`) : ($.getdata('opencard5') ? $.getdata('opencard5') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait5 ? process.env.openwait5 : `${openwait}`) : ($.getdata('openwait5') ? $.getdata('openwait5') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw5 ? process.env.opencard_draw5 : opencard_draw) : ($.getdata('opencard_draw5') ? $.getdata('opencard_draw5') : opencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(opencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[opencard5]为"true"')
    }
    if(opencard+"" != "true"){
      return
    }
...
async function run() {
  try {
    ...
    if(opencard_addSku+"" == "true"){
      ...
      console.log('如需加购请设置环境变量[opencard_addSku5]为"true"');
    }
    await takePostRequest('activity_load');
    if(opencard_draw+"" !== "0"){
      $.runFalag = true
    ...
    }else console.log('如需抽奖请设置环境变量[opencard_draw5]为"3" 3为次数');
