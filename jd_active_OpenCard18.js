...
2.开10张卡 成功开1张 获得5豆
3.关注5豆 
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw18]为"3"
...
All变量适用
————————————————
入口：[ 1.8~1.26 集年味卡，享年味盛宴 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=cd20220108tpovk25f5b40s07t7&shareUuid=603c6b4ae669418cb85dcf823a8e19a0)]

请求太频繁会被黑ip
过10分钟再执行

cron:30 14 27,9-26/3 1 *
============Quantumultx===============
[task_local]
#1.8~1.26 集年味卡，享年味盛宴
30 14 27,9-26/3 1 * https://github.com/wh1te3zzz/jdgaming/blob/main/jd_active_OpenCard18.js, tag=1.8~1.26 集年味卡，享年味盛宴, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku18 ? process.env.opencard_addSku18 : `${opencard_addSku}`) : ($.getdata('opencard_addSku18') ? $.getdata('opencard_addSku18') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard18 ? process.env.opencard18 : `${opencard}`) : ($.getdata('opencard18') ? $.getdata('opencard18') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait18 ? process.env.openwait18 : `${openwait}`) : ($.getdata('openwait18') ? $.getdata('openwait18') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw18 ? process.env.opencard_draw18 : opencard_draw) : ($.getdata('opencard_draw18') ? $.getdata('opencard_draw18') : opencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(opencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[opencard18]为"true"')
    }
    if(opencard+"" != "true"){
      return
    }
  ...
  $.shareUuid = "603c6b4ae669418cb85dcf823a8e19a0"
...
async function run() {
  try {
    ...
    }else console.log('如需抽奖请设置环境变量[opencard_draw18]为"3" 3为次数');
