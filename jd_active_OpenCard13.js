...
2.开10张卡 成功开1张 获得5豆 
3.关注5豆 
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw13]为"3" 
...
All变量适用 
———————————————— 
入口：[ 1.5~1.25 集“年味卡”，享年货盛宴 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=cd2022n4ci22tlj1s2w3245u1mk&shareUuid=2aaa24f12e244614a822541b9fe187d9)] 

请求太频繁会被黑ip 
过10分钟再执行 

cron:30 4 5-25/2,26 1 * 
============Quantumultx=============== 
[task_local] 
#1.5~1.25 集“年味卡”，享年货盛宴 
30 4 5-25/2,26 1 * https://github.com/wh1te3zzz/jdgaming/blob/main/jd_active_OpenCard13.js, tag=1.5~1.25 集“年味卡”，享年货盛宴, enabled=true 
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku13 ? process.env.opencard_addSku13 : `${opencard_addSku}`) : ($.getdata('opencard_addSku13') ? $.getdata('opencard_addSku13') : `${opencard_addSku}`); 
...
opencard = $.isNode() ? (process.env.opencard13 ? process.env.opencard13 : `${opencard}`) : ($.getdata('opencard13') ? $.getdata('opencard13') : `${opencard}`); 
...
openwait = $.isNode() ? (process.env.openwait13 ? process.env.openwait13 : `${openwait}`) : ($.getdata('openwait13') ? $.getdata('openwait13') : `${openwait}`); 
...
opencard_draw = $.isNode() ? (process.env.opencard_draw13 ? process.env.opencard_draw13 : opencard_draw) : ($.getdata('opencard_draw13') ? $.getdata('opencard_draw13') : opencard_draw); 
...
!(async () => { 
 if ($.isNode()) { 
 if(opencard+"" != "true"){ 
 console.log('如需执行脚本请设置环境变量[opencard13]为"true"') 
 } 
 if(opencard+"" != "true"){ 
 return 
 } 
 ...
 $.shareUuid = "2aaa24f12e244614a822541b9fe187d9" 
...
async function run() { 
 ...
 }else console.log('如需抽奖请设置环境变量[opencard_draw13]为"3" 3为次数'); 
