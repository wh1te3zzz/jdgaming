...
3.关注5豆 
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw3]为"3"
...
All变量适用
————————————————
入口：[ 12.20~1.5 双旦寻宝 玩转大富翁 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dfw2021121702rics9xrkw5mpka&shareUuid=8c15a74e1fa14b80bcc79a5492f6e0d4)]

请求太频繁会被黑ip
过10分钟再执行

cron:30 3 21-31/2,2-5/2 12,1 *
============Quantumultx===============
[task_local]
#12.20~1.5 双旦寻宝 玩转大富翁
30 3 21-31/2,2-5/2 12,1 * https://github.com/wh1te3zzz/jdgaming/blob/main/jd_active_OpenCard3.js, tag=12.20~1.5 双旦寻宝 玩转大富翁, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku3 ? process.env.opencard_addSku3 : `${opencard_addSku}`) : ($.getdata('opencard_addSku3') ? $.getdata('opencard_addSku3') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard3 ? process.env.opencard3 : `${opencard}`) : ($.getdata('opencard3') ? $.getdata('opencard3') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait3 ? process.env.openwait3 : `${openwait}`) : ($.getdata('openwait3') ? $.getdata('openwait3') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw3 ? process.env.opencard_draw3 : opencard_draw) : ($.getdata('opencard_draw3') ? $.getdata('opencard_draw3') : opencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(opencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[opencard3]为"true"')
    }
    if(opencard+"" != "true"){
      return
    }
  ...
  $.shareUuid = "8c15a74e1fa14b80bcc79a5492f6e0d4"
...
async function run() {
  try {
    ...
    }else console.log('如需抽奖请设置环境变量[opencard_draw3]为"3" 3为次数');
