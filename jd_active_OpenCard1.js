...
All变量适用
————————————————
入口：[ 12.10~12.25 狂欢日联合盛典 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dd20211210xgdby3zem7vp5y&shareUuid=fcce9569f3384f2e8a576447a4c9c142)]
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku1 ? process.env.opencard_addSku1 : `${opencard_addSku}`) : ($.getdata('opencard_addSku1') ? $.getdata('opencard_addSku1') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard1 ? process.env.opencard1 : `${opencard}`) : ($.getdata('opencard1') ? $.getdata('opencard1') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait1 ? process.env.openwait1 : `${openwait}`) : ($.getdata('openwait1') ? $.getdata('openwait1') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw1 ? process.env.opencard_draw1 : opencard_draw) : ($.getdata('opencard_draw1') ? $.getdata('opencard_draw1') : opencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(opencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[opencard1]为"true"')
    }
    if(opencard+"" != "true"){
      return
    }
  ...
  $.shareUuid = "fcce9569f3384f2e8a576447a4c9c142"
...
async function run() {
  try {
    ...
    }else console.log('如需抽奖请设置环境变量[opencard_draw1]为"3" 3为次数');
