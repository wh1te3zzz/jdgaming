...
2.开12张 成功开1张 可能获得5京豆
  全部开完获得1次抽奖
3.关注10京豆 获得1次抽奖
4.加购5京豆 获得1次抽奖(默认不加购 如需加购请设置环境变量[opencard_addSku14]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[opencard_draw14]为"3"
填写要抽奖的次数 不足已自身次数为准
opencard_draw14="3"
填非数字会全都抽奖
...
[task_local]
#1.8～1.15 联合开卡
30 3 8-15/3 1 * https://github.com/wh1te3zzz/jdgaming/blob/main/jd_active_OpenCard14.js, tag=1.8～1.15 联合开卡, enabled=true
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku14 ? process.env.opencard_addSku14 : `${opencard_addSku}`) : ($.getdata('opencard_addSku14') ? $.getdata('opencard_addSku14') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard14 ? process.env.opencard14 : `${opencard}`) : ($.getdata('opencard14') ? $.getdata('opencard14') : `${opencard}`);
...
openwait = $.isNode() ? (process.env.openwait14 ? process.env.openwait14 : `${openwait}`) : ($.getdata('openwait14') ? $.getdata('openwait14') : `${openwait}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw14 ? process.env.opencard_draw14 : opencard_draw) : ($.getdata('opencard_draw14') ? $.getdata('opencard_draw14') : opencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(opencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[opencard14]为"true"')
    }
    if(opencard+"" != "true"){
      return
    }
...
async function run() {
  try {
    ...
    if(opencard_addSku+"" == "true"){
      ...
      console.log('如需加购请设置环境变量[opencard_addSku14]为"true"');
    }
    await takePostRequest('activity_load');
    if(opencard_draw+"" !== "0"){
      $.runFalag = true
    ...
    }else console.log('如需抽奖请设置环境变量[opencard_draw14]为"3" 3为次数');
